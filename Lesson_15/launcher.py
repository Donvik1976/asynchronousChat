...
def main():
    ...
    while True:
        ...
        if action == 'q':
            ...
            print(
                'Убедитесь, что на сервере зарегистрировано необходимо '
                'количество клиентов с паролем 123456.')
            print(
                'Первый запуск может быть достаточно долгим из-за генерации '
                'ключей!')
            clients_count = int(
            ...
            for i in range(clients_count):
                process.append(
                    subprocess.Popen(
                        [python_executable, f'client.py', '-n', f'test{i + 1}',
                         '-p', '123456'],
                        creationflags=subprocess.CREATE_NEW_CONSOLE))
        elif action == 'x':
            while process:
