...
def gui_create_model(database):
    ...
    for row in list_users:
        ...
        ip_item.setEditable(False)
        port_item = QStandardItem(str(port))
        port_item.setEditable(False)
        # Уберём милисекунды из строки времени, т.к. такая точность не требуется.
        time_item = QStandardItem(str(time.replace(microsecond=0)))
        time_item.setEditable(False)
        list_model.appendRow([user_item, ip_item, port_item, time_item])
    return list_model
...
def create_stat_model(database):
    ...
    for row in hist_list:
        ...
        user_item = QStandardItem(user)
        user_item.setEditable(False)
        last_seen_item = QStandardItem(str(last_seen.replace(microsecond=0)))
        last_seen_item.setEditable(False)
        sent_item = QStandardItem(str(sent))
        sent_item.setEditable(False)
        recvd_item = QStandardItem(str(recvd))
        recvd_item.setEditable(False)
        list_model.appendRow([user_item, last_seen_item, sent_item, recvd_item])
    return list_model
